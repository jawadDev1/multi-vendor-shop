...
interface DashboardPageTemplateProps {
  shop: {
    ...
    totalApprovedShops: number;
    pendingShopRequests: number;
  };
}

const HokageDashboardPageTemplate = ({ shop }: DashboardPageTemplateProps) => {
  if (!shop) return;
    ...
    <SellerPageWrapper>
      <div className="grid grid-cols-3 gap-6">
        <div className="bg-white rounded-md shadow-xl py-4 px-4 ">
          ...
          <Title className="ml-3">{totalApprovedShops}</Title>
          <Link href={"/hokage/shops"}>
            <Subtitle2 className="text-azure-blue">View shops</Subtitle2>
          </Link>
        </div>
        ...
        <div className="bg-white rounded-md shadow-xl py-4 px-4 ">
          <div className="flex items-center gap-3 min-h-[52px] ">
            <FaJediOrder size={30} className="text-charcoal-gray" />
            <Content>Shop Requests</Content>
          </div>
          <Title className="ml-3">{pendingShopRequests}</Title>
          <Link href={"/hokage/shop-requests"}>
            <Subtitle2 className="text-azure-blue">View requests</Subtitle2>
          </Link>
        </div>
...
export default HokageDashboardPageTemplate;
